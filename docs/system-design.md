...
`workers/generator/index.ts` 是异步生成进程。Web API 创建 `GenerationJob` 后向 BullMQ 队列写入 `{ jobId }`，Worker 消费队列后原子领取数据库里的 `PENDING` 任务。图节点包括 `asset_analyzer -> planner -> coder -> reviewer -> publisher -> cost`，Reviewer 失败时会进入 `coder_revision`，多次失败后进入 `fallback_coder`。
...
- `JobArtifact`：Agent 间 artifact contract，保存规格、代码包、review 报告、发布结果和成本报告。
...
`Game.parentGameId` 和 `Game.sourceVersionId` 用于记录 Remix 来源；`GenerationJob.parentGameId` 和 `GenerationJob.remixSourceVersionId` 用于追踪生成任务从哪个游戏版本派生。`GenerationJob` 还记录审核结果、模型 usage token 和成本估算。
...
首页查询不再一次性加载所有游戏。`Game` 维护 `likeCount`、`favoriteCount` 和 `searchText` 冗余字段，数据库层使用 `(status, publishedAt)`、`(status, playCount)`、`(status, likeCount)` 组合索引，`tags` 使用 GIN index，`searchText` 使用 PostgreSQL trigram index。分页、排序、搜索和标签筛选都下推到 PostgreSQL；当数据继续扩大时，可以把 `searchText` 同步到 Meilisearch / Elasticsearch，PostgreSQL 仍作为权威数据源。
...
- Remix 派生和源版本追踪。
- `GameVersion` 版本管理。
- 生成成本统计：优先使用模型 API usage token，缺失时回退估算。
- 轻量内容审核和资源限额。
